    ...
    def train(self):
        ...
        while step < self.total_steps:
            ...
            if int((step/(self.num_envs * self.num_steps))%self.eval_cycle) == 0:
                total_reward, all_episode_rewards = self.evaluate_policy(min(self.num_levels,50))
                if self.print_output:
...
#                 next_obs = torch.from_numpy(next_obs)
                ...
                next_obs, reward, done, info = self.env.step(action)
                next_obs = torch.from_numpy(self.framestack.stacked_obs(next_obs,done))
...
#region evaluation        
    def evaluate_policy(self, 
        ...
        if h.nstack == 1:
            ...
            eval_framestack = VecFrameStack(eval_env, h.nstack)
            obs = eval_framestack.reset()
            obs = torch.from_numpy(obs)
...
#                 obs = torch.from_numpy(obs)
                ...
                obs, reward, done, info = eval_env.step(action)
                obs = torch.from_numpy(eval_framestack.stacked_obs(obs,done))
